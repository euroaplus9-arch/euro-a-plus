...
const translations = {
    ka: {
        ...
        map_title:"ჩვენი მდებარეობა",
        map_placeholder:"რუკა (დაამატეთ Google Maps)",
        social_title:"სოციალური ქსელები",
        social_subtitle:"გამოგვყევით სოციალურ ქსელებში",
        write_us:"მოგვწერეთ"
    },
    ru: {
        ...
        map_placeholder:"Карта (добавьте Google Maps)",
        social_title:"Социальные сети",
        social_subtitle:"Подпишитесь на нас в соцсетях",
    ...
    en: {
        ...
        map_placeholder:"Map (add Google Maps)",
    ...
    tr: {
        ...
        map_title:"Konumumuz",
        map_placeholder:"Harita (Google Maps ekleyin)",
    ...
    az: {
        ...
        map_title:"Yerləşməmiz",
        map_placeholder:"Xəritə (Google Maps əlavə edin)",
        social_title:"Sosial Media",
        social_subtitle:"Bizi sosial mediada izləyin",
        write_us:"Bizə yazın"
    },
    fa: {
        ...
        map_title:"موقعیت ما",
        map_placeholder:"نقشه (Google Maps را اضافه کنید)",
        social_title:"شبکه‌های اجتماعی",
        social_subtitle:"ما را در شبکه‌های اجتماعی دنبال کنید",
...
function updatePagination() {
    ...
    const totalPages = Math.ceil(totalVisible / 12);
    let html = `<button class="arrow active-arrow" onclick="changePage(-1)">‹</button>`;
    for (let i = 1; i <= Math.min(totalPages, 3); i++) {
        html += `<span class="page-num ${i === 1 ? 'active' : ''}">${i}</span>`;
    }
    if (totalPages > 3) {
        html += `<span class="page-num">...</span>`;
        html += `<span class="page-num">${totalPages}</span>`;
    }
    html += `<button class="arrow active-arrow" onclick="changePage(1)">›</button>`;
    container.innerHTML = html;
}
...
function changePage(direction) {
    ...
    const end = Math.min(start + itemsPerPage, totalVisible);
...
function updatePaginationButtons() {
    ...
    const totalPages = Math.ceil(totalVisible / 12);
    let html = `<button class="arrow ${currentPage > 1 ? 'active-arrow' : ''}" onclick="changePage(-1)">‹</button>`;
    for (let i = 1; i <= Math.min(totalPages, 3); i++) {
    ...
    html += `<button class="arrow ${currentPage < totalPages ? 'active-arrow' : ''}" onclick="changePage(1)">›</button>`;
    container.innerHTML = html;
}
...
const searchItems = [
    { id: 1, name: 'Amazon Light Gray', category: 'tiles', img: 'images/plitka.jpg' },
    { id: 2, name: 'Luna', category: 'tiles', img: 'images/plitkae.jpg' },
    { id: 3, name: 'Amazon Dark Gray', category: 'tiles', img: 'images/plitkaz.jpg' },
    { id: 4, name: 'WASP S-9 Universal', category: 'cement', img: 'images/cement1.jpg' },
    { id: 5, name: 'WASP S-7 Standard', category: 'cement', img: 'images/cement2.jpg' }
];
...
function showSuggestions(query) {
    ...
    if (matches.length === 0) {
        suggestionsContainer.innerHTML = `<div class="search-suggestion-empty">😕 ${t('no_results')}</div>`;
    ...
    matches.forEach(item => {
        ...
        div.innerHTML = `
            ...
            <span class="suggestion-name">${highlightMatch(item.name, trimmed)}</span>
            <span class="suggestion-category">${item.category === 'tiles' ? 'ფილა' : 'ცემენტი'}</span>
        `;
...
// Клик вне поиска → закрываем подсказки
